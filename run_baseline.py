#!/usr/bin/env python3
"""Run counterfactual story edits with vLLM offline batch inference."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Run the counterfactual dataset with vLLM continuous batching and "
            "write resumable JSONL results."
        )
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Results JSONL path. Defaults to the existing "
            "results/<model>-baseline-<mode>.jsonl convention."
        ),
    )
    parser.add_argument(
        "--limit",
...
def build_prompt(example):
    """Build the minimal baseline prompt without reference annotations."""
    return f"""Story:
{example["story"]}
    ...
    for offset, (example, prompt, example_seed, request_output) in enumerate(
        ...
        record = {
            **example,
            "model": args.model,
            "revision": revision,
            "condition": f"baseline_{mode_name}",
...
def main():
    ...
    output_path = (
        Path(args.output)
        if args.output
        else Path("results") / f"{model_short_name}-baseline-{mode_name}.jsonl"
    )
