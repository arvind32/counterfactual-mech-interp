#!/usr/bin/env python3
"""Verify and package the source traces for the three-anchor experiment."""
...
def verify_identity(record: dict[str, Any], selected: dict[str, Any]) -> None:
    ...
    if selected.get("condition") != "baseline_p5":
        raise RuntimeError(f"{selected['source_id']} is not baseline_p5")
    if selected.get("role") != "anchor_source":
        raise RuntimeError(f"{selected['source_id']} is not an anchor source")
    if selected.get("original_result") not in {"PASS", "FAIL"}:
        raise RuntimeError(f"invalid original result for {selected['source_id']}")
...
def build_package(selection_path: Path, output_path: Path) -> list[dict[str, Any]]:
    ...
    if (selection.get("sample_seed_start"), selection.get("sample_seed_end_inclusive")) != (
        ...
        raise RuntimeError("source selection must reserve seeds 3000 through 3029")
    ...
    if {row["selection"]["story_key"] for row in packaged} != {
        "madrid",
        "starry_night",
    }:
        raise RuntimeError("expected Madrid and Starry Night source families")
