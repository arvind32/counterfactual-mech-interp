...
RESULT_BASENAME = "three_anchor_generalization_rollouts"
ANCHORS = ("A", "A_STAR", "C")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "A, A*, and C in six held-out baseline_p5 families."
        )
    )
...
def validate_sources(sources: list[dict[str, Any]]) -> None:
    ...
    story_keys = [source["selection"]["story_key"] for source in sources]
...
def main() -> int:
    ...
    if not args.sources.is_file():
        raise FileNotFoundError(
            ...
            "trace_branching_pilot/continuation_4/prepare_sources.py first"
        )
