#!/usr/bin/env python3
"""Fail-fast validation for the random-location control experiment."""
...
def main() -> int:
    ...
    print("RANDOM-LOCATION CONTROL FAIL-FAST SMOKE TEST", flush=True)
    try:
        stage("1. Package and source integrity")
        required = [
            TRACE_ROOT / "run_ab.py",
            TRACE_ROOT / "history_presence.py",
            ROOT / "random_selection_config.json",
            ROOT / "prepare_random_sources.py",
            ROOT / "run_random_location_controls.py",
        ...
        import prepare_random_sources

        packaged = prepare_random_sources.build_package(
            ROOT / "random_selection_config.json",
            ROOT / "random_source_records.jsonl",
            PROJECT_ROOT / "results" / "trace_branching" /
            "random_location_selection_audit.json",
        )
        controls = [row["selection"]["random_control"] for row in packaged]
        if len({item["per_source_randomization_seed"] for item in controls}) != 12:
            raise RuntimeError("per-source randomization seeds are not unique")
        if len({item["selected_candidate_index_0_based"] for item in controls}) < 2:
            raise RuntimeError("all traces unexpectedly used the same candidate index")
        if len({item["text_sha256"] for item in controls}) != 12:
            raise RuntimeError("randomly selected line texts are not unique")
        passed(
            "sources",
            "12 pinned baseline_p5 traces with one independently randomized line each",
        )

        stage("2. Complete paired rollout design")
        import run_random_location_controls as runner

        sources = runner.load_jsonl(ROOT / "random_source_records.jsonl")
        jobs = runner.build_jobs(sources)
        runner.validate_design(jobs)
        expected_jobs = 12 * 1 * 2 * 30
        expected_pairs = 12 * 1 * 30
        ...
        passed(
            ...
            "before/after pairs; seeds 5000–5029",
        )
        ...
        if len(states) != 24:
            raise RuntimeError(f"expected 24 unique branch states, found {len(states)}")
        ...
        passed(
            "prefixes",
            f"24/24 exact unique prefix round trips; before/after differences "
            f"match only their randomly selected natural lines; budgets {budgets}",
        )

        stage("7. Generate briefly from all 24 branch states")
        ...
        passed("actual branches", "all 24 source/random-location/branch states generated tokens")
        ...
        print("Saved: trace_branching_pilot/continuations_5/smoke_report.json")
