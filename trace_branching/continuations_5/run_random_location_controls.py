#!/usr/bin/env python3
"""Run continuations immediately before and after random natural trace lines."""
...
RESULT_BASENAME = "random_location_control_rollouts"
ANCHORS = ("RANDOM",)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "one reproducibly selected random line in each of 12 traces."
        )
    )
    parser.add_argument(
...
def validate_sources(sources: list[dict[str, Any]]) -> None:
    if len(sources) != 12:
        raise RuntimeError(f"expected 12 source traces, found {len(sources)}")
    ids: list[str] = []
    ...
    for source_row in sources:
        ...
        for field in ("required_fact", "rubric_key", "rubric_summary"):
            if not isinstance(selected.get(field), str) or not selected[field]:
                raise RuntimeError(f"missing {field} for {source_id}")
        ...
        if selected.get("source_package") not in {
            "three_anchor_discovery",
            "three_anchor_generalization",
        }:
            raise RuntimeError(f"invalid source package for {source_id}")
        control = selected.get("random_control")
        if not isinstance(control, dict):
            raise RuntimeError(f"missing random control location for {source_id}")
        required_control_fields = (
            "global_randomization_seed",
            ...
            "overlaps_known_anchor",
        )
        missing = [field for field in required_control_fields if field not in control]
        if missing:
            raise RuntimeError(f"unresolved random location for {source_id}: {missing}")
        start = control["start_char_0_based"]
        end = control["end_char_0_based_exclusive"]
        after = control["after_char_0_based_exclusive"]
        if not all(isinstance(value, int) for value in (start, end, after)):
            raise RuntimeError(f"invalid random offsets for {source_id}")
        if not 0 < start < end <= after <= len(raw_completion):
            raise RuntimeError(f"invalid random interval for {source_id}")
        before_prefix = raw_completion[:start]
        after_prefix = raw_completion[:after]
        if len(before_prefix) != control["before_prefix_char_count"]:
            raise RuntimeError(f"before-prefix length mismatch for {source_id}")
        if len(after_prefix) != control["after_prefix_char_count"]:
            raise RuntimeError(f"after-prefix length mismatch for {source_id}")
        if raw_completion[start:end] != control["text"]:
            raise RuntimeError(f"random text mismatch for {source_id}")
        if control["text_sha256"] != hash_text(control["text"]):
            raise RuntimeError(f"random text hash mismatch for {source_id}")
        if control["global_randomization_seed"] != 20260904:
            raise RuntimeError(f"randomization seed mismatch for {source_id}")
        candidate_count = control["eligible_candidate_count"]
        candidate_index = control["selected_candidate_index_0_based"]
        if (
            not isinstance(candidate_count, int)
            or not isinstance(candidate_index, int)
            or not 0 <= candidate_index < candidate_count
        ):
            raise RuntimeError(f"invalid random candidate index for {source_id}")
        if after_prefix[len(before_prefix):] != control["inserted_text"]:
            raise RuntimeError(f"random insertion mismatch for {source_id}")
        if control["overlaps_known_anchor"] is not False:
            raise RuntimeError(f"random location overlaps a known anchor for {source_id}")
    ...
    origins = {source["selection"]["source_package"] for source in sources}
    if origins != {"three_anchor_discovery", "three_anchor_generalization"}:
        raise RuntimeError("expected sources from both three-anchor experiments")
...
def build_jobs(sources: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for source_index, source_row in enumerate(sources):
        ...
        if any(value.startswith("OVERLAPS_") for value in relations.values()):
            raise RuntimeError(
                f"random location overlaps a known anchor for {selected['source_id']}"
            )
        prefixes = {
                ...
                jobs.append(
                    {
                        ...
                        "eligible_candidate_count": control[
                            "eligible_candidate_count"
                        ],
                        "selected_candidate_index_0_based": control[
                            "selected_candidate_index_0_based"
                        ],
                        "random_line_number_1_based": control[
                            "line_number_1_based"
                        ],
                        "random_start_char_0_based": control[
                            "start_char_0_based"
                        ],
...
def main() -> int:
    ...
    if not args.sources.is_file():
        raise FileNotFoundError(
            ...
            "trace_branching_pilot/continuations_5/prepare_random_sources.py first"
        )
